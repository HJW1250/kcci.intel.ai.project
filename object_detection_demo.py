...
def draw_detections(frame, detections, palette, labels, output_transform):
    ...
    # 대체 이미지로 대체
        # 이미지 파일 경로
        image_path = r"C:\Users\teee\Media_Mosaic\ryan.png"

        # 이미지 파일 읽기
        replacement_image = cv2.imread(image_path)

        
        replacement_image_resized = cv2.resize(replacement_image, (xmax - xmin, ymax - ymin))
        frame[ymin:ymax, xmin:xmax] = replacement_image_resized
